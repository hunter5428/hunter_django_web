-- 동일한 직장주소를 가진 회원 조회 (휴대폰 번호 뒷자리 옵션)
-- 바인드 변수: :workplace_address, :workplace_detail_address, :phone_suffix, :current_cust_id
SELECT DISTINCT
    ...
    'WORKPLACE_ADDRESS' AS "MATCH_TYPE"
...
WHERE KYC_CUST_BASE.CUST_ID != :current_cust_id
  AND KYC_CUST_BASE.WPLC_ADDR IS NOT NULL
  AND KYC_CUST_BASE.WPLC_ADDR = :workplace_address
  AND (KYC_CUST_BASE.WPLC_DTL_ADDR = :workplace_detail_address OR :workplace_detail_address IS NULL)
