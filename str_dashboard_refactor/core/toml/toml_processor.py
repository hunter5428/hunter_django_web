...
class TomlDataProcessor:
    ...
    def __init__(self):
        from .toml_config import TomlFieldConfig
        self.config = TomlFieldConfig
    
    def mask_customer_field(self, field_name: str, value: Any, 
                           cust_id: str = None, mid: str = None) -> Any:
        """고객 정보 필드 마스킹"""
        config = self.config.CUSTOMER_INFO
        ...
        if field_name in config['partial_masking']:
            ...
            return self.mask_partial_value(field_name, value, rule)
        
        # 마스킹 대상이 아닌 경우 원본 반환
        return value
    
    def mask_partial_value(self, field_name: str, value: str, rule: str) -> str:
        """부분 마스킹"""
        ...
        if rule == 'district_only':
            # 주소를 구 단위까지만 표시
            return self._extract_district(value)
        
        elif rule == 'year_with_age':
            # 생년월일을 연도와 만 나이로 변환
            return self._format_birthdate_with_age(value)
    ...
    def _format_birthdate_with_age(self, birthdate: str) -> str:
        """생년월일을 연도와 만 나이로 변환"""
        if not birthdate:
            return birthdate
    ...
    def process_duplicate_matches(self, match_types: str) -> str:
        """중복 매칭 타입을 설명 문자열로 변환"""
        if not match_types:
            return "일치하는 정보가 없습니다"
        
        config = self.config.DUPLICATE_PERSON
    ...
    def extract_sspc_text(self, sspc_value: str) -> str:
    ...
    def substitute_rule_ids(self, rule_ids: List[str], rep_rule_id: str) -> Dict[str, str]:
    ...
    def format_orderbook_summary(self, patterns: Dict, period: Dict) -> str:
        """Orderbook 패턴을 텍스트 요약으로 변환"""
        config = self.config.ORDERBOOK_SUMMARY
        sentences = []
                        ...
                        if isinstance(item, tuple) and len(item) == 2:
                            ...
                            if isinstance(data, dict):
                                detail_str = config['template']['detail'].format(
                                    ticker=ticker,
                                    amount=self.format_amount(data.get('amount_krw', 0)),
    ...
    def format_stds_dtm_summary(self, stds_data: Dict) -> str:
        """STDS_DTM 요약을 텍스트로 변환"""
        config = self.config.STDS_DTM_SUMMARY
        ...
        # Orderbook 포맷 함수 재사용
        body = self.format_orderbook_summary(patterns, period)
    ...
    def format_ip_access_summary(self, ip_data: List[Dict]) -> str:
        """IP 접속 이력을 텍스트 요약으로 변환"""
        if not ip_data:
            return "IP 접속 이력이 없습니다"
        
        config = self.config.IP_ACCESS_HISTORY
    ...
    def process_related_person_transactions(self, transactions: List[Dict]) -> str:
        """관련인 거래 내역을 텍스트로 변환"""
        config = self.config.RELATED_PERSON['individual']
